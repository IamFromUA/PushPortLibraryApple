...
На стороне сервиса PushPort приложению должен соответствовать тот же Bundle ID и правильное окружение APNs. Версия SDK 0.0.1 подготовлена для серверного контракта из [protocol.md](protocol.md). В кабинете PushPort открой настройки приложения → Apple APNs. Загрузи .p8, укажи Team ID, Key ID и среду Sandbox или Production. При необходимости настрой обе среды отдельно. Бэк отправляет напрямую через APNs; Firebase для iOS не нужен. Перед рассылкой проверь получение на подписанном приложении.
